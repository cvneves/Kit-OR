...
void buscaVizinhancaSwap(std::vector<int> &s, double &valor_obj, double **m)
{
  ...
  {
    double perda_i = m[s[i]][s[i + 1]] + m[s[i - 1]][s[i]];
    ...
    for (int j = i + 1; j < s.size() - 1; j++)
    {
      ...
      if (j != i + 1)
      {
        delta = m[s[i - 1]][s[j]] + m[s[j]][s[i + 1]] + m[s[j - 1]][s[i]] + m[s[i]][s[j + 1]] - perda_i - m[s[j - 1]][s[j]] - m[s[j]][s[j + 1]];
      }
      else
      {
      ...
      // std::vector<int> s_copia = s;
      // swap(s_copia, i, j);

      // double objPorco = calcularValorObj(s_copia, matrizAdj);

      // if (calcularValorObj(s_copia, matrizAdj) != valor_obj + delta)
      // {
      //   std::cout << " i,j = " << i << ", " << j << "\n";
      // }
      if (delta < 0)
...
void buscaVizinhancaReinsertion(std::vector<int> &solucao, double &valor_obj, double **m, int tam_subsequencia)
{
  ...
  for (int i = 1; i < solucao.size() - tam_subsequencia; i++)
  {
    ...
    for (int j = 1; j < solucao.size() - tam_subsequencia; j++)
    {
      ...
      if (i > j)
      {
        // delta = m[solucao[i]][solucao[j - 1]] + m[solucao[i + tam_subsequencia - 1]][solucao[j]] - m[solucao[j - 1]][solucao[j]] + perda_1;
        delta = -m[solucao[j - 1]][solucao[j]] - m[solucao[i - 1]][solucao[i]] - m[solucao[i + tam_subsequencia - 1]][solucao[i + tam_subsequencia]] + m[solucao[i - 1]][solucao[i + tam_subsequencia]] + m[solucao[j - 1]][solucao[i]] + m[solucao[i + tam_subsequencia - 1]][solucao[j]];
      }
      else
      {
        // continue;
        ...
        delta = -m[*(rit + j)][*(rit + j - 1)] - m[*(rit + i)][*(rit + i - 1)] - m[*(rit + i + tam_subsequencia)][*(rit + i + tam_subsequencia - 1)] + m[*(rit + i + tam_subsequencia)][*(rit + i - 1)] + m[*(rit + i)][*(rit + j - 1)] + m[*(rit + j)][*(rit + i + tam_subsequencia - 1)];
      ...
      // std::vector<int> s_copia = solucao;

      // reinsertion(s_copia, i, tam_subsequencia, j);

      // double objPorco = calcularValorObj(s_copia, matrizAdj);

      // if (calcularValorObj(s_copia, matrizAdj) != valor_obj + delta)
      // {
      //   printSolucao(s_copia);
      //   std::cout << "reinsertion i,j, t = " << i << ", " << j << ", " << tam_subsequencia << "\n";
      //   std::cout << "delta: " << delta << "\n";
      //   std::cout << "deltaPorco: " << calcularValorObj(s_copia, matrizAdj) - valor_obj << "\n";

      //   {
      //     std::cout << "- " << solucao[j - 1] << ", " << solucao[j] << "\n";
      //     std::cout << "- " << solucao[i - 1] << ", " << solucao[i] << "\n";
      //     std::cout << "- " << solucao[i + tam_subsequencia - 1] << ", " << solucao[i + tam_subsequencia] << "\n";

      //     std::cout << "+ " << solucao[i - 1] << ", " << solucao[i + tam_subsequencia] << "\n";
      //     std::cout << "+ " << solucao[j - 1] << ", " << solucao[i] << "\n";
      //     std::cout << "+ " << solucao[i + tam_subsequencia - 1] << ", " << solucao[j] << "\n\n\n";
      //   }
      // }
