...
int main(int argc, char **argv)
{
  ...
  // std::vector<std::vector<double>> matrizDistancia = {{1, 30, 26, 50, 40}, {30, 1, 24, 40, 50}, {26, 24, 1, 24, 26}, {50, 40, 24, 1, 30}, {40, 50, 26, 30, 1}};

  std::vector<int> s = GILS_RVND(), cl;
  std::vector<int> lista_candidatos;
  std::vector<std::pair<std::pair<int, int>, double>> custo_insercao;
  double valor_obj;
  //construcao(s, lista_candidatos, 7, custo_insercao, valor_obj, 0.3, matrizAdj, dimension);
  ...
  while (tree.empty() == false)
  {
    ...
    for (auto &aresta : node->s)
    {
      ...
      if (aresta.first == noEscolhido || aresta.second == noEscolhido)
      {
        ...
        n.calculateLB(dimension, matrizDistancia, UB);
        tree.push_back(n);
      }
    }
    tree.erase(node);
